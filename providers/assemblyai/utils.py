...
def get_transcript(polling_endpoint, header):
    transcript = requests.get(polling_endpoint, headers=header)
    return transcript.json()


# Get speaker exchanges
def get_speaker_exchanges(transcript):
    exchanges = []

    for utterance in transcript['utterances']:
        speaker = "Agent" if utterance['channel'] == "1" else "Customer"
        start_time_formatted = timedelta(seconds=utterance['start']/1000)
        exchanges.append(f"{speaker} ({start_time_formatted}): {utterance['text']}")
